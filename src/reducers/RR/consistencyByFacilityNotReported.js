...
export default (state = initialState, action) => {
    let newState = { ...state };
    switch (action.type) {
        case actionTypes.RR_CONSISTENCY_BY_FACILITY_NOT_REPORTED_REQUEST:
            newState.loading[action.payload.docket] = true;
            newState.lastFetch[action.payload.docket] = Date.now();
            return newState;
        case actionTypes.RR_CONSISTENCY_BY_FACILITY_NOT_REPORTED_FETCH:
            ...
            newState.lastFetch[action.payload.docket] = Date.now();
            return newState;
        case actionTypes.RR_CONSISTENCY_BY_FACILITY_NOT_REPORTED_REQUEST_FAILED:
            newState.loading[action.payload.docket] = false;
