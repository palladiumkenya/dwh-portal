...
export const loadCurrentOnArtVerifiedByCounty = (tab) => async (dispatch, getState) => {
    if (getState().filters.noCache === true) {
        await dispatch(fetchCurrentOnArtByAgeSex());
    } else {
        const diffInMinutes = moment().diff(
            moment(getState().currentOnArtVerifiedByCounty.lastFetch),
            'minutes'
        );
        if (
...
export const fetchCurrentOnArtByAgeSex = () => async (dispatch, getState) => {
    dispatch({ type: actionTypes.CT_TX_CURR_COUNTY_VERIFIED_REQUEST });
    const params = {
    ...
    dispatch({ type: actionTypes.CT_TX_CURR_COUNTY_VERIFIED_FETCH, payload: { filtered: getState().filters.filtered, list: response }});
};
