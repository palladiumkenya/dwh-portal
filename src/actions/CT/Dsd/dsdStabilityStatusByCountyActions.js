...
export const loadDsdStabilityStatusByCounty = (tab) => async (dispatch, getState) => {
    const diffInMinutes = moment().diff(
        moment(getState().dsdStabilityStatusByCounty.lastFetch),
        'minutes'
    );
    if (getState().ui.ctTab !== 'dsd' && tab !== 'dsd' && getState().ui.currentPage !== PAGES.home) {
        ...
        await dispatch(fetchDsdStabilityStatusByCounty());
    }
};

export const fetchDsdStabilityStatusByCounty = () => async (dispatch, getState) => {
    dispatch({ type: actionTypes.CT_DSD_STABILITY_STATUS_BY_COUNTY_REQUEST });
    const params = {
