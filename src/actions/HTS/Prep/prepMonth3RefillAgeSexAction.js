...
export const loadPrepMonth3RefillAgeSex = () => async (dispatch, getState) => {
    const diffInMinutes = moment().diff(
        moment(getState().prepMonth3RefillAgeSex.lastFetch),
        'minutes'
    );
...
export const fetchCTPrep = () => async (dispatch, getState) => {
    dispatch({ type: actionTypes.CT_PREP_MONTH3_REFILL_AGE_SEX_REQUEST });
    const params = {
    ...
    try {
        const response = await axios.get(
            `${DWH_API_URL}hts/getPrepRefillAgeSexMonth3`,
            {
                params: params,
            }
        );
        dispatch({
            type: actionTypes.CT_PREP_MONTH3_REFILL_AGE_SEX_FETCH,
            payload: {
        ...
        dispatch({ type: actionTypes.CT_PREP_MONTH3_REFILL_AGE_SEX_FAILED });
    }
};
