...
export const loadPrepScreeningOutcome = () => async (dispatch, getState) => {
    const diffInMinutes = moment().diff(
        moment(getState().prepScreeningOutcome.lastFetch),
        'minutes'
    );
...
export const fetchCTPrep = () => async (dispatch, getState) => {
    dispatch({ type: actionTypes.PREP_SCREENING_OUTCOME_REQUEST });
    const params = {
    ...
    try {
        const response = await axios.get(
            `${DWH_API_URL}hts/getPrepSTIScreeningOutcome`,
            {
                params: params,
            }
        );
        dispatch({ type: actionTypes.PREP_SCREENING_OUTCOME_FETCH, payload: { filtered: getState().filters.filtered, list: response.data }});
    } catch (e) {
        dispatch({ type: actionTypes.PREP_SCREENING_OUTCOME_FAILED });
    }
};
