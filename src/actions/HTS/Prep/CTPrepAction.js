...
export const loadCTPrep = () => async (dispatch, getState) => {
    const diffInMinutes = moment().diff(
        moment(getState().ctPrep.lastFetch),
        'minutes'
    );
...
export const fetchCTPrep = () => async (dispatch, getState) => {
    dispatch({ type: actionTypes.CT_PREP_REQUEST });
    const params = {
    ...
    try {
        const response = await axios.get(`${DWH_API_URL}hts/getCtPrep`, {
            params: params,
        });
        dispatch({ type: actionTypes.CT_PREP_FETCH, payload: { filtered: getState().filters.filtered, list: response.data }});
    } catch (e) {
        dispatch({ type: actionTypes.CT_PREP_FAILED });
    }
};
