...
export const loadMissedEIDMissingPCR = () => async (dispatch, getState) => {
    const lastFetch = getState().missedEIDMissingPCR.lastFetch;
...
export const fetchMissedEID = () => async (dispatch, getState) => {
    const previousMonth = moment()
        ...
        .add(15, 'days');
    dispatch({
        type: actionTypes.PMTCT_RRI_MISSED_EID_MISSING_PCR_REQUEST,
    });
    const params = {
    ...
    try {
        const response = await getAll(
            'pmtct-rri/getMissedEIDMissingPCR',
            params
        );
        dispatch({
            type: actionTypes.PMTCT_RRI_MISSED_EID_MISSING_PCR_FETCH,
            payload: {
        ...
        dispatch({
            type: actionTypes.PMTCT_RRI_MISSED_EID_MISSING_PCR_FAILED,
        });
    }
};
