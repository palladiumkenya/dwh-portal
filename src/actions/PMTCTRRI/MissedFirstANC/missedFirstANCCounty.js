...
export const loadMissedFirstANCCounty = () => async (dispatch, getState) => {
    const lastFetch = getState().missedFirstANCCounty.lastFetch;
...
export const fetchMissedFirstANC = () => async (dispatch, getState) => {
    const previousMonth = moment()
        ...
        .add(15, 'days');
    dispatch({
        type: actionTypes.PMTCT_RRI_MISSED_FIRST_ANC_COUNTY_REQUEST,
    });
    const params = {
    ...
    try {
        const response = await getAll('pmtct-rri/getMissedANCByCounty', params);
        dispatch({
            type: actionTypes.PMTCT_RRI_MISSED_FIRST_ANC_COUNTY_FETCH,
            payload: {
        ...
        dispatch({
            type: actionTypes.PMTCT_RRI_MISSED_FIRST_ANC_COUNTY_FAILED,
        });
    }
};
