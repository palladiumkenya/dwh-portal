...
export const loadMissedFirstANCOverview = () => async (dispatch, getState) => {
    const lastFetch = getState().missedFirstANCOverview.lastFetch;
...
export const fetchMissedFirstANC = () => async (dispatch, getState) => {
    const previousMonth = moment()
        ...
        .add(15, 'days');
    dispatch({
        type: actionTypes.PMTCT_RRI_MISSED_FIRST_ANC_OVERVIEW_REQUEST,
    });
    const params = {
    ...
    try {
        const response = await getAll('pmtct-rri/getMissedANCOverview', params);
        dispatch({
            type: actionTypes.PMTCT_RRI_MISSED_FIRST_ANC_OVERVIEW_FETCH,
            payload: {
        ...
        dispatch({
            type: actionTypes.PMTCT_RRI_MISSED_FIRST_ANC_OVERVIEW_FAILED,
        });
    }
};
