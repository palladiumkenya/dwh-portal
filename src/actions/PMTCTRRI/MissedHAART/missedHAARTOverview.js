...
export const loadMissedHAARTOverview = () => async (dispatch, getState) => {
    const lastFetch = getState().missedHAARTOverview.lastFetch;
...
export const fetchMissedHAART = () => async (dispatch, getState) => {
    const previousMonth = moment()
        ...
        .add(15, 'days');
    dispatch({
        type: actionTypes.PMTCT_RRI_MISSED_MATERNAL_HAART_OVERVIEW_REQUEST,
    });
    const params = {
    ...
    try {
        const response = await getAll('pmtct-rri/getMissedHAARTOverview', params);
        dispatch({
            type: actionTypes.PMTCT_RRI_MISSED_MATERNAL_HAART_OVERVIEW_FETCH,
            payload: {
        ...
        dispatch({
            type: actionTypes.PMTCT_RRI_MISSED_MATERNAL_HAART_OVERVIEW_FAILED,
        });
    }
};
