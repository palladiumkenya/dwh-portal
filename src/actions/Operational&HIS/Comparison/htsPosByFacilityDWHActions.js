...
export const loadHTSPosByFacilityDWH = () => async (dispatch, getState) => {
    const diffInMinutes = moment().diff(
        moment(getState().htsPosByFacilityDWH.lastFetch),
        'minutes'
    );
    ...
    if (getState().ui.currentPage !== PAGES.operationalHIS) {
        ...
        await dispatch(fetchHTSPosByFaciltyDWH());
    }
};

export const fetchHTSPosByFaciltyDWH = () => async (dispatch, getState) => {
    dispatch({ type: actionTypes.DWH_HTS_POS_BY_FACILITY_REQUEST });
    const params = {
    ...
    const response = await getAll(
        'operational-his/getDWHHTSPOSByFacility',
        params
    );
    dispatch({
        type: actionTypes.DWH_HTS_POS_BY_FACILITY_FETCH,
