...
export const loadHTSPosByCountyKHIS = () => async (dispatch, getState) => {
    const diffInMinutes = moment().diff(
        moment(getState().htsPosByCountyKHIS.lastFetch),
        'minutes'
    );
    ...
    if (getState().ui.currentPage !== PAGES.operationalHIS) {
        ...
        await dispatch(fetchHTSPosByCountyKHIS());
    }
};

export const fetchHTSPosByCountyKHIS = () => async (dispatch, getState) => {
    dispatch({ type: actionTypes.KHIS_HTS_POS_BY_COUNTY_REQUEST });
    const params = {
    ...
    const response = await getAll(
        'operational-his/getKHISHTSPOSByCounty',
        params
    );
    dispatch({
        type: actionTypes.KHIS_HTS_POS_BY_COUNTY_FETCH,
