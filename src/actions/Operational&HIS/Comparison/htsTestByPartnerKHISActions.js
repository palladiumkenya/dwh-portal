...
export const loadHTSTestByPartnerKHIS = () => async (dispatch, getState) => {
    const diffInMinutes = moment().diff(
        moment(getState().htsTestByPartnerKHIS.lastFetch),
        'minutes'
    );
    ...
    if (getState().ui.currentPage !== PAGES.operationalHIS) {
        ...
        await dispatch(fetchHTSTestByPartnerKHIS());
    }
};

export const fetchHTSTestByPartnerKHIS = () => async (dispatch, getState) => {
    dispatch({ type: actionTypes.KHIS_HTS_TEST_BY_PARTNER_REQUEST });
    const params = {
    ...
    const response = await getAll(
        'operational-his/getKHISHTSTESTByPartner',
        params
    );
    dispatch({
        type: actionTypes.KHIS_HTS_TEST_BY_PARTNER_FETCH,
