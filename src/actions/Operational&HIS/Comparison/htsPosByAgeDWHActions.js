...
export const loadHTSPosByAgeDWH = () => async (dispatch, getState) => {
    const diffInMinutes = moment().diff(
        moment(getState().htsPosByAgeDWH.lastFetch),
        'minutes'
    );
    ...
    if (getState().ui.currentPage !== PAGES.operationalHIS) {
        ...
        await dispatch(fetchHTSPosByAgeDWH());
    }
};

export const fetchHTSPosByAgeDWH = () => async (dispatch, getState) => {
    dispatch({ type: actionTypes.DWH_HTS_POS_BY_AGE_REQUEST });
    const params = {
    ...
    const response = await getAll('operational-his/getDWHHTSPOSByAge', params);
    dispatch({
        type: actionTypes.DWH_HTS_POS_BY_AGE_FETCH,
