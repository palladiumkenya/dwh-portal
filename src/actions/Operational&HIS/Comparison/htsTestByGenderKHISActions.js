...
export const loadHTSTestByGenderKHIS = () => async (dispatch, getState) => {
    const diffInMinutes = moment().diff(
        moment(getState().htsTestByGenderKHIS.lastFetch),
        'minutes'
    );
    ...
    if (getState().ui.currentPage !== PAGES.operationalHIS) {
        ...
        await dispatch(fetchHTSTestByGenderKHIS());
    }
};

export const fetchHTSTestByGenderKHIS = () => async (dispatch, getState) => {
    dispatch({ type: actionTypes.KHIS_HTS_TEST_BY_GENDER_REQUEST });
    const params = {
    ...
    const response = await getAll(
        'operational-his/getKHISHTSTEST',
        params
    );
    dispatch({
        type: actionTypes.KHIS_HTS_TEST_BY_GENDER_FETCH,
