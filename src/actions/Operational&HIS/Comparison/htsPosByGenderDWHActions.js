...
export const loadHTSPosByGenderDWH = () => async (dispatch, getState) => {
    const diffInMinutes = moment().diff(
        moment(getState().htsPosByGenderDWH.lastFetch),
        'minutes'
    );
    ...
    if (getState().ui.currentPage !== PAGES.operationalHIS) {
        ...
        await dispatch(fetchHTSPosByGenderDWH());
    }
};

export const fetchHTSPosByGenderDWH = () => async (dispatch, getState) => {
    dispatch({ type: actionTypes.DWH_HTS_POS_BY_GENDER_REQUEST });
    const params = {
    ...
    const response = await getAll(
        'operational-his/getDWHHTSPOSByGender',
        params
    );
    dispatch({
        type: actionTypes.DWH_HTS_POS_BY_GENDER_FETCH,
