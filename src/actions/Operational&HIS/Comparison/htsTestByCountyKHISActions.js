...
export const loadHTSTestByCountyKHIS = () => async (dispatch, getState) => {
    const diffInMinutes = moment().diff(
        moment(getState().htsTestByCountyKHIS.lastFetch),
        'minutes'
    );
    ...
    if (getState().ui.currentPage !== PAGES.operationalHIS) {
        ...
        await dispatch(fetchHTSTestByCountyKHIS());
    }
};

export const fetchHTSTestByCountyKHIS = () => async (dispatch, getState) => {
    dispatch({ type: actionTypes.KHIS_HTS_TEST_BY_COUNTY_REQUEST });
    const params = {
    ...
    const response = await getAll(
        'operational-his/getKHISHTSTESTByCounty',
        params
    );
    dispatch({
        type: actionTypes.KHIS_HTS_TEST_BY_COUNTY_FETCH,
