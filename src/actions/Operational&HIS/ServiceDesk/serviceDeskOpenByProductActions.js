...
export const loadServiceDeskOpenByProduct = () => async (dispatch, getState) => {
    const diffInMinutes = moment().diff(
        moment(getState().openByProduct.lastFetch),
        'minutes'
    );
...
export const fetchServiceDesk = () => async (dispatch, getState) => {
    dispatch({ type: actionTypes.SERVICE_DESK_OPEN_BY_PRODUCT_REQUEST });
    const params = {
    ...
    const response = await getAll(
        'operational-his/getOpenIssuesByProduct',
        params
    );
    dispatch({ type: actionTypes.SERVICE_DESK_OPEN_BY_PRODUCT_FETCH, payload: { filtered: getState().filters.filtered, list: response }});
};
