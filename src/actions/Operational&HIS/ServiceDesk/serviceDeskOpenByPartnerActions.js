...
export const loadServiceDeskOpenByPartner = () => async (dispatch, getState) => {
    const diffInMinutes = moment().diff(
        moment(getState().openByPartner.lastFetch),
        'minutes'
    );
...
export const fetchServiceDesk = () => async (dispatch, getState) => {
    dispatch({ type: actionTypes.SERVICE_DESK_OPEN_BY_PARTNER_REQUEST });
    const params = {
    ...
    const response = await getAll(
        'operational-his/getOpenIssuesByPartner',
        params
    );
    dispatch({ type: actionTypes.SERVICE_DESK_OPEN_BY_PARTNER_FETCH, payload: { filtered: getState().filters.filtered, list: response }});
};
