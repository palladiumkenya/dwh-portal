...
export const loadServiceDeskOverview = () => async (dispatch, getState) => {
    const diffInMinutes = moment().diff(
        moment(getState().ticketsOverview.lastFetch),
        'minutes'
    );
...
export const fetchServiceDeskOverview = () => async (dispatch, getState) => {
    dispatch({ type: actionTypes.SERVICE_DESK_OVERVIEW_REQUEST });
    const params = {
    ...
    const response = await getAll('operational-his/getTicketsOverview', params);
    dispatch({ type: actionTypes.SERVICE_DESK_OVERVIEW_FETCH, payload: { filtered: getState().filters.filtered, list: response }});
};
