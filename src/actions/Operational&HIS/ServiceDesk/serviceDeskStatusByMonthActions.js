...
export const loadServiceDeskStatusByMonth = () => async (dispatch, getState) => {
...
export const fetchServiceDesk = () => async (dispatch, getState) => {
    dispatch({ type: actionTypes.SERVICE_DESK_STATUS_BY_MONTH_REQUEST });
    const params = {
    ...
    const response = await getAll(
        'operational-his/getIssueStatusByMonth',
        params
    );
    dispatch({ type: actionTypes.SERVICE_DESK_STATUS_BY_MONTH_FETCH, payload: { filtered: getState().filters.filtered, list: response }});
};
