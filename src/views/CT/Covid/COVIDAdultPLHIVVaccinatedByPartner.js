...
const COVIDAdultPLHIVVaccinatedByPartner = () => {
    ...
    const loadVaccinatedByPartner = useCallback(async () => {
        setCovidVaccinatedByPartner({
            ...
            series: [
                { name: 'PARTIALLY VACCINATED', data: partnersVaccinated.partiallyVaccinated, type: 'column', color: "#F08532", tooltip: { valueSuffix: ' ({point.percentage:.0f}%)' } },
                { name: 'FULLY VACCINATED', data: partnersVaccinated.fullyVaccinated, type: 'column', color: "#69B34C", tooltip: { valueSuffix: ' ({point.percentage:.0f}%)' } },
            ]
        });
    }, []);
