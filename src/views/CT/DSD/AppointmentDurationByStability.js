...
const AppointmentDurationByStability = ({ globalFilter }) => {
    ...
    const loadAppointmentDurationByStability = useCallback(async () => {
        ...
        const periodGroups = ["STABLE", "UNSTABLE"];
        let firstPeriod = [];
        ...
        result.forEach(function (res) {
            let index = periodGroups.indexOf("STABLE");
            ...
            index = periodGroups.indexOf("UNSTABLE");
            if (res.period === 'Same Day') {
                firstPeriod.splice(index, 0, parseInt(res.txNew));
            } else if (res.period === '1 to 7 Days') {
                secondPeriod.splice(index, 0, parseInt(res.txNew));
            ...
            <div className="col-12">
                <Card className="trends-card">
                    <CardHeader className="trends-header">
                        APPOINTMENT DURATION PRACTICES BY STABILITY STATUS (N =495)
                    </CardHeader>
