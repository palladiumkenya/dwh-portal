    ...
    const loadAppointmentDurationByStability = useCallback(async () => {
        setAppointmentDurationByStability({
            ...
            series: [
                ...
                { name: '3-5 MONTHS', data: appointmentDurationByStabilityStatus.data[1], type: 'column', color: "#2F4050", tooltip: { valueSuffix: ' ({point.percentage:.0f}%)' } },
                { name: '6+ MONTHS', data: appointmentDurationByStabilityStatus.data[2], type: 'column', color: "#3281CC", tooltip: { valueSuffix: ' ({point.percentage:.0f}%)' } },
            ]
        });
