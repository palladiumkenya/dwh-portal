...
export const getProportionOfAlhivEnrolledInOtzWhoHaveCompletedTrainingFemales = createSelector(
    [listUnfiltered, listFiltered, filtered],
    (listUnfiltered, listFiltered, filtered) => {
        const list = filtered ? listFiltered : listUnfiltered;

        const arrayVal = [];
        const trainedValues = list.filter(obj => obj.training === 1 && obj.Gender === 'Female');
        const unTrainedValues = list.filter(obj => obj.training === 0 && obj.Gender === 'Female');
