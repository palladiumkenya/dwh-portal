...
const listUnfiltered = (state) =>
    state.adverseEventsReportedWithSeverityLevels.listUnfiltered;
const listFiltered = (state) =>
    state.adverseEventsReportedWithSeverityLevels.listFiltered;
...
export const getAdverseEventsReportedWithSeverityLevelsTabs = (list, tab) => {
    const ageGroup =
        tab === 'adult'
            ? ageGroups.adultAgeGroups
            : ageGroups.childrenAgeGroups;
    ...
    categories = categories.filter((s) => {
        return s.adverseEvent !== 'LIPODYSTROPHY';
    });

    categories = categories
        .filter((obj) => obj.total >= 30)
        .map((y) => y.adverseEvent);
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        let categoryIndex = categories.indexOf(
            list[i].AdverseEvent.toUpperCase()
        );
        ...
        data[severityIndex][categoryIndex] =
            data[severityIndex][categoryIndex] + parseInt(list[i].total);
    }

    return {
        categories: categories.map((c) =>
            c === 'OTHER ADRS' ? 'OTHER AEs' : c
        ),
        severityCategories,
        data,
    };
