...
const listUnfiltered = state => state.currentOnArtDistributionByCounty.listUnfiltered;
const listFiltered = state => state.currentOnArtDistributionByCounty.listFiltered;
const filtered = state => state.filters.filtered;

export const getCurrentOnArtDistributionByCounty = createSelector(
    [listUnfiltered, listFiltered, filtered],
    (listUnfiltered, listFiltered, filtered) => {
        const list = filtered ? listFiltered : listUnfiltered;
        const data = [];
        const counties = _.uniqBy(list, obj => obj.County);
        for (let i = 0; i < counties.length; i++) {
            ...
            const lessThan5 = list.filter(obj => obj.County === counties[i].County && (obj.ageGroup === "<1" || obj.ageGroup === "1-4"));
            const fiveToNine = list.filter(obj => obj.County === counties[i].County && (obj.ageGroup === "5-9"));
            const tenTo14 = list.filter(obj => obj.County === counties[i].County && (obj.ageGroup === "10-14"));
            const fifteenTo19 = list.filter(obj => obj.County === counties[i].County && (obj.ageGroup === "15-19"));
            const twentyTo24 = list.filter(obj => obj.County === counties[i].County && (obj.ageGroup === "20-24"));
            const twentyFiveTo34 = list.filter(obj => obj.County === counties[i].County && (obj.ageGroup === "25-29" || obj.ageGroup === "30-34"));
            const thirtyFiveTo44 = list.filter(obj => obj.County === counties[i].County && (obj.ageGroup === "35-39" || obj.ageGroup === "40-44"));
            const fourtyFiveTo49 = list.filter(obj => obj.County === counties[i].County && (obj.ageGroup === "45-49"));
            const fiftyplus = list.filter(obj => obj.County === counties[i].County && (obj.ageGroup === "50+"));

            const maleGroup = list.filter(obj => obj.CTPartner === counties[i].CTPartner && (obj.Gender === "Male" || obj.Gender === "M"));
            const feMaleGroup = list.filter(obj => obj.CTPartner === counties[i].CTPartner && (obj.Gender === "Female" || obj.Gender === "F"));
